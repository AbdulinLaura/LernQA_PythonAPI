...
class Test_API:
  ...
  @pytest.mark.parametrize('user_agent,expected_result', data)
  def test_headers(self, user_agent, expected_result):
        headers = {'User-Agent': user_agent}
        response = requests.get("https://playground.learnqa.ru/ajax/api/user_agent_check", headers=headers)
        obj = response.json()
        print(obj)
        assert obj["platform"] == expected_result["platform"], "Platform is not correct"
        assert obj["browser"] == expected_result["browser"], "Browser is not correct"
        assert obj["device"] == expected_result["device"], "Device is not correct"
